...
function timed_finish(label, next, before_next){
    var t0 = new Date;
 ...
 *          If the key isn't found, get(key) returns null but get([keys]) returns [].
 *
...
Postgres.prototype.wait_for_txn = function(){
    var self = this;
    var __func = 'Postgres[' + self.__table + ']';
    try{
        if (self.__txn_in_progress){
            log('info', __func + ' txn in progress; waiting');
    ...
    return self.__txn_in_progress;
}

Postgres.prototype.put = function(key, value, next){
    var self = this;
    if (self.wait_for_txn()){
        return;
    }
    if (!self.__client){
        pg_reconnect(self, function(err){
    ...
    self.__txn_in_progress = true;
    var done = timed_finish(__func, next, function(){ self.__txn_in_progress = false });
    ...
    // This is fine for Postgres 9.3, but fails on 9.1 and older. When the initial
    // UPDATE fails and you plow on with INSERT it says "current transaction is
    ...
    self.__client.query(
        ...
        if_ok(function(){
            self.__client.query(
                ...
                if_ok(function(data){
                    if (data.rowCount > 0){
                        log('debug', __func + ' - update succeeded on first try');
                        return self.__client.query('commit', done);
                    }
                    log('debug', __func + ' - update failed, trying insert');
                    self.__client.query(
                        'savepoint alpha',
                        if_ok(function(){
                            self.__client.query(
                                ...
                                if_ok(function(data){
                                    if (data.rowCount > 0){
                                        ...
                                        log('debug', __func + ' - insert succeeded');
                                        return self.__client.query('commit', done);
                                    }
                                    log('debug', __func + ' - insert failed, trying update again');
                                    self.__client.query(
                                        ...
                                        if_ok(function(){
                                            self.__client.query(
                                                ...
                                                [value, key],
                                                if_ok(function(data){
                                                    log('debug', __func + ' - second insert: ' + JSON.stringify(data));
                                                    return self.__client.query('commit', done);
                                                })
                                            );
                                        })
    ...
    self.__client.query(
        ...
        function(err, data){
            ...
            if (keys.length === 1){
                ...
                result = result.length < 1 ? null : result[0];
            }
            done(null, result);
        }
    );
