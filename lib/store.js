 ...
 *
 *  configuration:
 *      storage.type = [MemStore | Redis | Postgres ...]
 *
 *  import:
...
function pg_reconnect(store, done){
    var name = "postgress://" + store.__config.user + ':<pass>@' +
                store.__config.host + ':' + store.__config.port + '/' + store.__config.database;
    log('debug', 'pg_reconnect(' + name + ')');
    if (store.__client){
        try{ store.__client.end(); }
        catch(ex){ /* don't care */ }
    }
    store.__client = new pg.Client(store.__config);
...
function Postgres(table, conf){
    ...
    pg_reconnect(self, function(err){
        if (err){
            log('error', 'Postgres[' + self.__table + '] failed to connect to DB');
        }
    });
}
...
Postgres.prototype.get = function(keys, next){
    process.nextTick(function(){ done('Not implemented'); }); // TODO
}
