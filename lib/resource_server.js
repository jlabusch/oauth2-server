...
var dummy = {
    ...
    // This function allows you to pass additional information to the EJS views.
    set_view_context: function(ctx){
        return ctx;
    }
};
...
// Also uses resource_server options "host", "port", "basic_auth, "user_agent", "allow_bad_ssl_cert".
var stuff_nation = {
    api: {
        scopes: ['read-only'], // If you add scopes, you must update the messaging in views/dialog.ejs
        profile: {
            scope: 'read-only',
            fn: function(req, res){
                var conf = config.get('resource_server');
                // Don't need to check req.authInfo.scope; this uses only the most basic scope.
                var opt = {
                    hostname: conf.host,
                    port: conf.port,
                    path: '/_json/account_info?rememberme_token=' + req.authInfo.site_token,
                    method: 'GET',
                    rejectUnauthorized: conf.allow_bad_ssl_cert !== true,
                    headers: {
                        'User-Agent': conf.user_agent,
                        'Accept': '*/*'
                    }
        ...
        var req = https.request(opt, function(res){
            ...
            res.on('end', function(){
                ...
                if (res.statusCode === 200){
                        ...
                        try{
                            ...
                            if (x.status === 'error' && x.error){
                                switch(x.error.type){
                                    case undefined:
                                    case null:
                                        x.error.msg = x.error.msg || stock_error;
                                        break;
                                    case 'not_confirmed_user':
                                        x.error.msg = 'Account not yet active';
                                        break;
                                    case 'deactivated_user':
                                        x.error.msg = 'Account deactivated';
                                        break;
                                    case 'not_https':
                                        x.error.msg = stock_error;
                                        break;
                                    case 'legacy_user':
                                        x.error.msg = 'EssentialMums/Stuff Nation account not yet linked.';
                                        break;
                                    case 'forgot_password':
                                        x.error.msg = x.error.msg || 'Invalid email or password';
                                        break;
                                    default:
                                        log('warn', 'Unhandled Stuff Nation error type ' + x.error.type);
                                        x.error.msg = x.error.msg || stock_error;
                                        break;
                                }
                    ...
                    if (uid && uid.length > 1){
                        uid = create_hash(uid[1]);
                    }else{
                        log('warn', 'Couldn\'t parse UID out of rememberme token "' + rememberme + '"');
                        uid = 'unknown';
                    }
                    user.id = uid;
    ...
    set_view_context: function(ctx){
        var conf = config.get('resource_server');
        ctx.domain = conf.host.match(/^[^.]+\.(\S*)/);
        ctx.domain = ctx.domain ? ctx.domain[1] : 'stuff.co.nz';
        return ctx;
    }
};
...
exports.load_context_fn = loader('set_view_context');
...
function create_hash(str){
    ...
    try{
        ...
        log('error', "Couldn't calculate SHA1 hash for \"" + str + "\" - " + ex);
    }
    return str;
}
